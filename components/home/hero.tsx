...
export default function Hero() {
  ...
  return (
    <section className='relative h-[95vh] pt-20 flex items-center'>
      {/* Main content container */}
      <div className='container mx-auto px-4 relative z-10 h-full flex items-center'>
        <div className='flex flex-col-reverse lg:flex-row w-full gap-8 items-center'>
          {/* Left side - Text content */}
          <div
            className={`w-full lg:w-1/2 space-y-5 max-w-2xl transition-all duration-1000 ${
              isLoaded
            ...
            <Badge
              variant='outline'
              className='px-4 py-1.5 bg-primary/10 dark:bg-primary/20 border-0 rounded-full'>
              <span className='text-primary dark:text-white font-medium text-sm flex items-center'>
                <CheckCircle2 className='w-4 h-4 mr-2 text-tawakal-green animate-pulse' />
            ...
            <p className='text-base md:text-lg max-w-none text-muted-foreground dark:text-zinc-300'>
            ...
            <div className='flex flex-row gap-2 pt-1 w-full'>
              <Link href='#products' className='flex-1'>
                <Button
                  size='lg'
                  className='w-full bg-gradient-to-r from-tawakal-green to-tawakal-blue hover:from-tawakal-blue hover:to-tawakal-green text-white rounded-full px-3 py-2 md:px-6 md:py-5 text-xs md:text-base shadow-md hover:shadow-lg transition-all duration-300 group relative overflow-hidden'>
              ...
              <Link href='#agents' className='flex-1'>
                <Button
                  variant='outline'
                  size='lg'
                  className='w-full border-tawakal-blue/20 hover:border-tawakal-blue hover:bg-tawakal-blue/5 dark:border-zinc-700 dark:hover:bg-zinc-800 text-foreground dark:text-white rounded-full px-3 py-2 md:px-6 md:py-5 text-xs md:text-base group'>
            ...
            <div className='pt-1'>
              <p className='text-muted-foreground dark:text-zinc-400 text-xs md:text-sm mb-1.5 flex items-center'>
                <Globe className='w-3.5 h-3.5 mr-1.5 text-tawakal-green' />
              ...
              <div className='flex flex-wrap gap-1.5'>
                {countries.map((country, index) => (
                  <span
                    key={country.name}
                    className={`px-2.5 py-0.5 rounded-full text-xs md:text-sm transition-all duration-300 ${
                      index === activeCountry
